...
class ViewController: UIViewController, MKMapViewDelegate {
	...
	override func viewDidLoad() {
        ...
        center.addObserver(self, selector: "userDefaultsChanged:", name: NSUserDefaultsDidChangeNotification, object: userDefaults)
        userDefaultsChanged(nil)        // simulate a change to perform initial setup
    ...
    func userDefaultsChanged(notification: NSNotification?) {
        ...
        if userDefaults.boolForKey(PreferenceKey.CloudSync.rawValue) {
            // The user has selected to synchronize the location with other devices via iCloud
            if cloudStore == nil {
                // cloudStore needs to be set up and initialized
            ...
            // The user has choosen to not synchronize locations using iCloud
            if cloudStore != nil {
                // The existing cloud store object needs to be torn down
                center.removeObserver(self, name: nil, object: cloudStore)
                cloudStore = nil
            }
        }
    }
